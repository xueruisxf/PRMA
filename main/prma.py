...
from model.prma import Bert_Encoder, FFN_Layer
...
import logging
from utils import set_seed, confusion_matrix_view, select_data, get_proto, get_aca_data, select_data_random, get_mem_aug
...
def train_balance_model(config, encoder, classifier, training_data, seen_relations, rel2id, epochs, prev_lr=0.001, cur_lr=0.001, t=0):
    data_loader = get_data_loader(config, training_data, shuffle=True)
    encoder.train()
    classifier.train()
    ...
    for epoch_i in range(epochs):
        ...
        for step, (_, labels, sentences, _, _) in enumerate(tqdm(data_loader)):
            ...
            # loss = criterion(logits, labels)
            if t == 0:
                loss = criterion_o(logits, labels)
            else:
                loss = criterion(config, t, logits, labels, fisher, encoder, model_old)
            losses.append(loss.item())
            loss.backward()
            torch.nn.utils.clip_grad_norm_(encoder.parameters(), config.max_grad_norm)
            torch.nn.utils.clip_grad_norm_(classifier.parameters(), config.max_grad_norm)
...
def criterion(config, t, output, targets, fisher, model, model_old):
    cen = nn.CrossEntropyLoss()
    loss_reg = 0
    if t > 0:
    ...
    return cen(output, targets)+config.lamb * loss_reg


def get_fisher_all(t, xtrain, ytrain, fisher, model, classifier):
    ...
    model.train()
    for i in tqdm(range(0, xtrain.size(0), 32), desc='Fisher Compute', ncols=100, ascii=True):
        b = torch.LongTensor(np.arange(i, np.min([i + 32, xtrain.size(0)]))).cuda()
        input_x = torch.autograd.Variable(xtrain[b], volatile=False)
        target = torch.autograd.Variable(ytrain[b], volatile=False)

        model.zero_grad()
        classifier.zero_grad()
        reps = model(input_x)
        outputs = classifier.prev_forward(reps)
        loss = ce_c(outputs, target)
        loss.backward()

        for n, p in model.named_parameters():
            if p.grad is not None:
                fisher[n] += 32 * p.grad.data.pow(2)

    for n, _ in model.named_parameters():
        fisher[n] = fisher[n]/xtrain.size(0)
        fisher[n] = torch.autograd.Variable(fisher[n], requires_grad=False)

    if t > 0:
        for n, _ in model.named_parameters():
            fisher[n] = (fisher[n] + fisher_old[n] * t) / (t + 1)
    return fisher
...
if __name__ == '__main__':
    ...
    log_path = os.path.join(config.log_dir, "{}".format(config.exp_name) + "_lamb{}".format(config.lamb) + 'del35.txt')
    ...
    for i in range(config.total_round):
        ...
        for episode, (training_data, _, test_data, current_relations, historic_test_data, seen_relations,
            ...
            if episode == 0:
                ...
                train_balance_model(config, encoder, classifier, train_data_for_initial, seen_relations, rel2id, config.step1_epochs, prev_lr=0.00001, t=episode)
            ...
            if episode == 3:
                del_rel = current_relations
            if episode == 5:
                del_rel += current_relations
            if episode >= 3:
                ma = deepcopy(memorized_samples)
                for rel in del_rel:
                    ma.pop(rel)
                mem_aug = get_mem_aug(ma, tokenizer)
            else:
                mem_aug = get_mem_aug(memorized_samples, tokenizer)
            mem_data += mem_aug    
            ...
            logger.info('Computer Weight')
            model_old = deepcopy(encoder)
            model_old.eval()
            for param in model_old.parameters():
                param.requires_grad = False

            data_loader = get_data_loader(config, train_data_for_initial, shuffle=True, batch_size=4000)
            current_relation_ids = [rel2id[rel] for rel in current_relations]
            for step, (_, labels, sentences, _, _) in enumerate(tqdm(data_loader)):
                sentences = torch.stack([x.to(config.device) for x in sentences], dim=0)
                labels = torch.tensor([current_relation_ids.index(i.item()) for i in labels]).long()  # 0 ~ rel_per_task
            ...
            model_comp = deepcopy(encoder)
            fisher = get_fisher_all(episode, sentences, labels, fisher, model_comp, classifier)
