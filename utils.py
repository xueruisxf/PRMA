...
def get_mem_aug(mem_data_s, tokenizer):
    mem_data = deepcopy(mem_data_s)
    mem_aug_data = []
    for rel, data in mem_data.items():
        if rel in ['P26', 'P3373', 'per:siblings', 'org:alternate_names', 'per:spous', 'per:alternate_names', 'per:other_family']:
            for item in mem_data[rel]:
    ...
    for rel, data in mem_data.items():
        ...
        for item in data:
            rel_id = item['relation']
            token_sen = item['tokens'][:]
            random.shuffle(data)
            if data[0] == item:
                token_en = data[1]['tokens'][:]
            else:
                token_en = data[0]['tokens'][:]

            h_tokens = token_en[token_en.index(30522)+1:token_en.index(30523)]
            t_tokens = token_en[token_en.index(30524)+1:token_en.index(30525)]
            token_sen[token_sen.index(30522) + 1: token_sen.index(30523)] = h_tokens
            token_sen[token_sen.index(30524) + 1: token_sen.index(30525)] = t_tokens

            mem_aug_data.append({
                'relation': rel_id,
                'tokens': token_sen,
                'string': tokenizer.decode(token_sen)
            })
            for index in [30522, 30523, 30524, 30525]:
                assert index in token_sen and token_sen.count(index) == 1
    return mem_aug_data
